...
def profile():
    ...
    ticket_stats = {
        'created': Ticket.query.filter_by(user_id=user.id).count(),
        'open': Ticket.query.filter_by(user_id=user.id, status='Open').count(),
        'resolved': Ticket.query.filter_by(user_id=user.id, status='Resolved').count()
    }
    
    if user.role == 'agent':
        ticket_stats['assigned'] = Ticket.query.filter_by(assigned_to=user.id).count()
        ticket_stats['agent_resolved'] = Ticket.query.filter_by(assigned_to=user.id, status='Resolved').count()
    
    return render_template_string(PROFILE_TEMPLATE, user=user, ticket_stats=ticket_stats)
...
def toggle_user_status(user_id):
    ...
    db.session.commit()
    logger.info(f"Admin {session['username']} {status} user {user.username}")
...
def delete_user(user_id):
    ...
    # Unassign agent tickets
    if user.role == 'agent':
        agent_tickets = Ticket.query.filter_by(assigned_to=user.id).all()
    ...
    flash(f'User {username} has been deleted. Their tickets have been reassigned to you.', 'success')
    return redirect(url_for('manage_users'))
...
def change_user_role(user_id):
    ...
    logger.info(f"Admin {session['username']} changed {user.username} role from {old_role} to {new_role}")
...
def reports():
    ...
    agent_performance = []
    for agent in agents:
        total = Ticket.query.filter_by(assigned_to=agent.id).count()
        resolved = Ticket.query.filter_by(assigned_to=agent.id, status='Resolved').count()
        avg_time = 0
        resolved_tickets = Ticket.query.filter_by(
            assigned_to=agent.id
        ).filter(Ticket.resolved_at.isnot(None)).all()
        
        if resolved_tickets:
            total_time = sum([(t.resolved_at - t.created_at).total_seconds() / 3600 
                            for t in resolved_tickets])
            avg_time = round(total_time / len(resolved_tickets), 2)
...
def sla_monitor():
    ...
    return render_template_string(SLA_MONITOR_TEMPLATE,
        user=user,
        overdue_tickets=overdue_tickets,
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .dropdown-toggle:hover { background: #5568d3; }
        .dropdown-content {
            display: none;
            position: absolute;
            right: 0;
    ...
    <nav class="navbar">
        ...
        <div class="dropdown">
            ...
            <div class="dropdown-content">
                <div class="dropdown-header">{{ session.role }}</div>
                ...
                <a href="{{ url_for('sla_monitor') }}">⏰ SLA Monitor</a>
                ...
                <a href="{{ url_for('logout') }}" style="color: #f56565;">🚪 Logout</a>
            </div>
        </div>
    </nav>
    {% endif %}
...
VIEW_ARTICLE_TEMPLATE = BASE_TEMPLATE.replace('{% block content %}{% endblock %}', '''
{% block content %}
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
    <h2>{{ article.title }}</h2>
    <a href="{{ url_for('knowledge_base') }}" class="btn" style="background: #e2e8f0; color: #2d3748;">Back to Knowledge Base</a>
</div>
...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; padding-bottom: 1rem; border-bottom: 2px solid #e2e8f0;">
        <div>
            ...
            <span style="color: #718096; margin-left: 1rem;">{{ article.views }} views</span>
        </div>
        <div style="color: #718096;">
            <small>Created by {{ article.user.username }} on {{ article.created_at.strftime('%Y-%m-%d') }}</small>
        </div>
    </div>
            ...
            <tr>
                ...
                <td>
                    <form method="POST" action="{{ url_for('change_user_role', user_id=user.id) }}" style="display: inline;">
                        <select name="role" onchange="this.form.submit()" style="padding: 0.3rem; border: 1px solid #ddd; border-radius: 3px;" {% if user.id == session.user_id %}disabled{% endif %}>
                ...
                <td>
                    Created: {{ user.tickets_created|length }}<br>
                    {% if user.role == 'agent' %}Assigned: {{ user.tickets_assigned|length }}{% endif %}
                </td>
                <td>
                    {% if user.id != session.user_id %}
                    <div style="display: flex; gap: 0.5rem;">
                        <form method="POST" action="{{ url_for('toggle_user_status', user_id=user.id) }}" style="display: inline;">
                            <button type="submit" class="btn" style="padding: 0.4rem 0.8rem; background: {% if user.is_active %}#ed8936{% else %}#48bb78{% endif %}; color: white;">
                                {% if user.is_active %}Deactivate{% else %}Activate{% endif %}
                            </button>
                        </form>
                        <form method="POST" action="{{ url_for('delete_user', user_id=user.id) }}" onsubmit="return confirm('Are you sure you want to delete this user? This action cannot be undone.');" style="display: inline;">
                            <button type="submit" class="btn btn-danger" style="padding: 0.4rem 0.8rem;">Delete</button>
                        </form>
                    </div>
                    {% else %}
                    <span style="color: #718096; font-size: 0.9rem;">Current User</span>
                    {% endif %}
                </td>
            </tr>
            {% endfor %}
...
<div class="card">
    <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 2rem;">
        <div style="text-align: center; padding: 2rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 10px; color: white;">
            <div style="width: 120px; height: 120px; border-radius: 50%; background: white; margin: 0 auto 1rem; display: flex; align-items: center; justify-content: center; font-size: 3rem; color: #667eea;">
                {{ user.username[0].upper() }}
            </div>
            <h3>{{ user.username }}</h3>
            <span class="badge" style="background: rgba(255,255,255,0.3); color: white; margin-top: 0.5rem;">{{ user.role }}</span>
        </div>
        ...
        <div>
            ...
            <div style="display: grid; gap: 1rem;">
                <div style="background: #f7fafc; padding: 1rem; border-radius: 5px;">
                    <strong style="color: #718096; font-size: 0.9rem;">Email</strong>
                    <p style="margin-top: 0.3rem;">{{ user.email }}</p>
                </div>
                <div style="background: #f7fafc; padding: 1rem; border-radius: 5px;">
                    <strong style="color: #718096; font-size: 0.9rem;">Role</strong>
                    <p style="margin-top: 0.3rem;">{{ user.role|title }}</p>
                </div>
                <div style="background: #f7fafc; padding: 1rem; border-radius: 5px;">
                    <strong style="color: #718096; font-size: 0.9rem;">Member Since</strong>
                    <p style="margin-top: 0.3rem;">{{ user.created_at.strftime('%B %d, %Y') }}</p>
                </div>
                <div style="background: #f7fafc; padding: 1rem; border-radius: 5px;">
                    <strong style="color: #718096; font-size: 0.9rem;">Account Status</strong>
...
<div class="card">
    <h3 style="margin-bottom: 1.5rem; color: #667eea;">Ticket Statistics</h3>
    <div class="stats-grid">
        <div class="stat-card" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
            <h3>{{ ticket_stats.created }}</h3>
            <p>Tickets Created</p>
        </div>
        <div class="stat-card" style="background: linear-gradient(135deg, #48bb78 0%, #38a169 100%);">
            <h3>{{ ticket_stats.open }}</h3>
            <p>Open Tickets</p>
        </div>
        <div class="stat-card" style="background: linear-gradient(135deg, #4299e1 0%, #3182ce 100%);">
        ...
        {% if user.role == 'agent' %}
        <div class="stat-card" style="background: linear-gradient(135deg, #ed8936 0%, #dd6b20 100%);">
            <h3>{{ ticket_stats.assigned }}</h3>
            <p>Assigned to Me</p>
        </div>
        <div class="stat-card" style="background: linear-gradient(135deg, #9f7aea 0%, #805ad5 100%);">
            <h3>{{ ticket_stats.agent_resolved }}</h3>
            <p>Resolved by Me</p>
        </div>
        {% endif %}
    </div>
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
    <h2>Edit Profile</h2>
    <a href="{{ url_for('profile') }}" class="btn" style="background: #e2e8f0; color: #2d3748;">← Back to Profile</a>
</div>
...
<div class="card">
    <form method="POST">
        ...
        <div style="border-top: 2px solid #e2e8f0; margin: 2rem 0; padding-top: 2rem;">
            <h3 style="margin-bottom: 1.5rem; color: #667eea;">Change Password</h3>
            <p style="color: #718096; margin-bottom: 1rem; font-size: 0.9rem;">Leave blank if you don't want to change your password</p>
        ...
        <div style="display: flex; gap: 1rem; margin-top: 2rem;">
            ...
            <a href="{{ url_for('profile') }}" class="btn" style="background: #e2e8f0; color: #2d3748;">Cancel</a>
        </div>
    </form>
</div>
{% endblock %}
...
{% block content %}
<h2 style="margin-bottom: 2rem;">📊 Admin Reports & Analytics</h2>
...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; margin-bottom: 2rem;">
    <div class="card">
        <h3 style="margin-bottom: 1.5rem; color: #667eea;">Tickets by Category</h3>
        {% for category, count in category_stats %}
        <div style="background: #f7fafc; padding: 1rem; margin-bottom: 0.5rem; border-radius: 5px; display: flex; justify-content: space-between; align-items: center;">
            <span>{{ category }}</span>
            <span class="badge" style="background: #667eea; color: white;">{{ count }}</span>
        </div>
        {% endfor %}
    </div>
    
    <div class="card">
        <h3 style="margin-bottom: 1.5rem; color: #667eea;">Tickets by Priority</h3>
        {% for priority, count in priority_stats %}
        <div style="background: #f7fafc; padding: 1rem; margin-bottom: 0.5rem; border-radius: 5px; display: flex; justify-content: space-between; align-items: center;">
            <span>{{ priority }}</span>
            <span class="badge" style="background: 
                {% if priority == 'Critical' %}#f56565
                {% elif priority == 'High' %}#ed8936
                {% elif priority == 'Medium' %}#ecc94b
                {% else %}#48bb78{% endif %}; color: white;">{{ count }}</span>
        </div>
        {% endfor %}
    </div>
</div>
...
<div class="card">
    <h3 style="margin-bottom: 1.5rem; color: #667eea;">Agent Performance</h3>
    <table>
        <thead>
            <tr>
                <th>Agent</th>
...
SLA_MONITOR_TEMPLATE = BASE_TEMPLATE.replace('{% block content %}{% endblock %}', '''
{% block content %}
<h2 style="margin-bottom: 2rem;">⏰ SLA Monitoring Dashboard</h2>

<div class="stats-grid" style="grid-template-columns: repeat(2, 1fr); margin-bottom: 2rem;">
    <div class="stat-card" style="background: linear-gradient(135deg, #f56565 0%, #e53e3e 100%);">
        <h3>{{ overdue_tickets|length }}</h3>
        <p>Overdue Tickets</p>
    </div>
    <div class="stat-card" style="background: linear-gradient(135deg, #ed8936 0%, #dd6b20 100%);">
        <h3>{{ approaching_sla|length }}</h3>
        <p>Approaching SLA (24h)</p>
    </div>
</div>

{% if overdue_tickets %}
<div class="card">
    <h3 style="margin-bottom: 1.5rem; color: #f56565;">🚨 Overdue Tickets</h3>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr style="background: #fff5f5;">
                    <td><strong>{{ ticket.ticket_id }}</strong></td>
                    <td>{{ ticket.subject[:40] }}...</td>
                    <td><span class="badge" style="background: 
                        {% if ticket.priority == 'Critical' %}#f56565
                        {% elif ticket.priority == 'High' %}#ed8936
                        {% else %}#ecc94b{% endif %}; color: white;">{{ ticket.priority }}</span></td>
                    <td>{{ ticket.created_at.strftime('%Y-%m-%d %H:%M') }}</td>
                    <td>{{ ticket.sla_deadline.strftime('%Y-%m-%d %H:%M') }}</td>
                    <td style="color: #f56565; font-weight: 600;">
                        {{ ((now - ticket.sla_deadline).total_seconds() / 3600)|round(1) }}h
                    </td>
                    <td><a href="{{ url_for('view_ticket', ticket_id=ticket.ticket_id) }}" class="btn btn-primary" style="padding: 0.4rem 0.8rem;">View</a></td>
                </tr>
                {% endfor %}
...
<div class="card" style="text-align: center; padding: 3rem; color: #718096;">
    <h3>✅ No Overdue Tickets</h3>
    <p>All tickets are within SLA timeframes!</p>
</div>
{% endif %}

{% if approaching_sla %}
<div class="card">
    <h3 style="margin-bottom: 1.5rem; color: #ed8936;">⚠️ Tickets Approaching SLA Deadline</h3>
    <div style="overflow-x: auto;">
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr style="background: #fffaf0;">
                    ...
                    <td>{{ ticket.created_at.strftime('%Y-%m-%d %H:%M') }}</td>
                    <td>{{ ticket.sla_deadline.strftime('%Y-%m-%d %H:%M') }}</td>
                    <td style="color: #ed8936; font-weight: 600;">
                        {{ ((ticket.sla_deadline - now).total_seconds() / 3600)|round(1) }}h
                    </td>
                    <td><a href="{{ url_for('view_ticket', ticket_id=ticket.ticket_id) }}" class="btn btn-primary" style="padding: 0.4rem 0.8rem;">View</a></td>
                </tr>
                {% endfor %}
...
def init_database():
    ...
    with app.app_context():
        ...
        if not User.query.filter_by(username='Underrated').first():
            # Create default admin user
            admin = User(
                username='Underrated',
                email='Paisonkatongola009720@gmail.com',
                password_hash=generate_password_hash('Sh3zzl3m@c'),
                role='admin'
            )
...
- Network connectivity problems''',
            ...
            print("-" * 60)
            print("Admin Account:")
            print("  Username: Underrated")
            print("  Password: Sh3zzl3m@c")
            print("\nAgent Account:")
